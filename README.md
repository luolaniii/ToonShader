...
完整 UE 引擎源码体积较大，实际引擎侧改动已提交到 UE fork 的独立分支：

- 仓库：`luolaniii/UnrealEngine`
- 分支：`toon-shader-modifications`
- 基线：`release`
- 内容：只包含 Toon shading model 和 Toon outline 所需的引擎源码改动文件，没有上传整个 zip 解包目录。
...
该引擎分支没有提交完整 UE 源码包，也没有提交本地生成文件、工程缓存或 IDE 文件。

未上传的内容包括本地缓存、中间产物、IDE 文件、辅助脚本、插件、笔试要求、压缩包、视频和其他非必要交付文件。
