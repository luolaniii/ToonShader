...
{
	return (1u << MSM_SubsurfaceProfile) | (1u << MSM_Eye) | (Substrate::IsSubstrateEnabled() ? (1u << MSM_PreintegratedSkin) : 0u);
}
...
{
	DECLARE_TYPE_LAYOUT(FMaterialCacheTagStack, NonVirtual);

public:
	friend bool operator==(const FMaterialCacheTagStack& Lhs, const FMaterialCacheTagStack& Rhs)
	{
		return Lhs.TagGuid != Rhs.TagGuid;
	}

	friend inline bool operator!=(const FMaterialCacheTagStack& Lhs, const FMaterialCacheTagStack& Rhs)
	{
		return !operator==(Lhs, Rhs);
	}
...
public:
	...
	{
		return Lhs.ParameterInfo == Rhs.ParameterInfo && Lhs.TextureCollectionIndex == Rhs.TextureCollectionIndex && Lhs.bIsVirtualCollection == Rhs.bIsVirtualCollection;
	}
	friend inline bool operator!=(const FMaterialTextureCollectionParameterInfo& Lhs, const FMaterialTextureCollectionParameterInfo& Rhs)
	{
		return !operator==(Lhs, Rhs);
...
public:
	...
	/// Allocates the specified number of float4s in the uniform buffer and returns the index
	/// of the first newly allocated float4 in the buffer.
	/// Note: multiply returned value by sizeof(float4) to know the byte offset in the buffer.
	uint32 AllocateFromUniformBuffer(uint32 NumOfFloat4s);
	
	TConstArrayView<FMaterialCacheTagStack> GetMaterialCacheTagStacks() const { return MaterialCacheTagStacks; }
	...
	/// @param BufferOffset Specifies the location in the preshader buffer where the result of the preshader instruction will be written to. See FMaterialUniformPreshaderField::BufferOffset.
	/// @param EntryType Specifies the value type of the preshader instruction output. See FMaterialUniformPreshaderField::Type.
	/// @param EntryGenerator Callback that writes the preshader opcodes.
...
protected:
	...
	/** Material cache tag stacks found during translation */
	LAYOUT_FIELD(TMemoryImageArray<FMaterialCacheTagStack>, MaterialCacheTagStacks)
...
Updates to layout fields in this class must be accompanied with a GUID bump to MaterialTranslationDDCVersion entry in FDevSystemGuids.
*/
...
public:
	FMaterialCompilationOutput() :
		...
		NumVirtualTextureFeedbackRequests(0),
#endif
	...
	// Returns Id for a user scene texture input, or INDEX_NONE if not found.
	int32 FindUserSceneTexture(FName UserSceneTexture) const;
	...
	{
		return UsedSceneTextures & GetGBufferMask();
	}
...
public:
	...
	/**
	 * Preferred format for the shader byte code the shader map is going to be compiled for.
	 * This is used for preview platforms and affects all shaders, so make it part of the shader map Id.
	 */
...
public:
	...
	*/
	...
	bool UsesTemporalResponsiveness() const { return GetContent()->MaterialCompilationOutput.bUsesTemporalResponsiveness; }
	bool UsesMotionVectorWorldOffset() const { return GetContent()->MaterialCompilationOutput.bUsesMotionVectorWorldOffset; }
	...
	ESubstrateBsdfFeature GetSubstrateMaterialBsdfFeatures() const { return ESubstrateBsdfFeature(GetSubstrateMaterialCompilationOutput().SubstrateMaterialBsdfFeatures); }
	ESubstrateTileType GetSubstrateTileType() const { return GetSubstrateTileTypeFromMaterialType(GetSubstrateMaterialCompilationOutput().SubstrateMaterialType); }
...
UE_DEPRECATED(5.8, "Use the GetMaterialValueTypeDescriptions(EMaterialValueType...) overload instead")
...
UE_DEPRECATED(5.8, "Use the CanConnectMaterialValueTypes(EMaterialValueType, EMaterialValueType) overload instead")
extern ENGINE_API bool CanConnectMaterialValueTypes(const uint32 InputType, const uint32 OutputType);
extern ENGINE_API bool CanConnectMaterialValueTypes(const EMaterialValueType InputType, const EMaterialValueType OutputType);
...
public:	
...
#else
	inline uint32 AddRef() const { return 0u; }
	inline uint32 Release() const { return 0u; }
	inline uint32 GetRefCount() const { return 0u; }
#endif
	 ...
	 */
	UE_DEPRECATED(5.7, "Call CacheShaders without EShaderPlatform as its now cached inside the FMaterial")
	 ...
	 * This is used by material resources of UMaterials.
	 */
	UE_DEPRECATED(5.7, "Call BeginCacheShaders without EShaderPlatform as its now cached inside the FMaterial")
	...
	ENGINE_API void BeginCacheShaders(EMaterialShaderPrecompileMode PrecompileMode = EMaterialShaderPrecompileMode::Default, const ITargetPlatform* TargetPlatform = nullptr, TUniqueFunction<void(bool bSuccess)>&& CompletionCallback = nullptr);
	...
	ENGINE_API void BeginCacheShaders(const FMaterialShaderMapId& ShaderMapId, EMaterialShaderPrecompileMode PrecompileMode = EMaterialShaderPrecompileMode::Default, const ITargetPlatform* TargetPlatform = nullptr, TUniqueFunction<void(bool bSuccess)>&& CompletionCallback = nullptr);
	...
	ENGINE_API void CacheGivenTypes(const TArray<const FVertexFactoryType*>& VFTypes, const TArray<const FShaderPipelineType*>& PipelineTypes, const TArray<const FShaderType*>& ShaderTypes);
	 ...
	 */
	UE_DEPRECATED(5.7, "Call CollectPSOs without ERHIFeatureLevel::Type as its now cached inside the FMaterial")
	 ...
	 */
	UE_DEPRECATED(5.7, "Call ShouldCachePipeline without EShaderPlatform as its now cached inside the FMaterial")
	ENGINE_API virtual bool ShouldCache(EShaderPlatform Platform, const FShaderType* ShaderType, const FVertexFactoryType* VertexFactoryType) const final;
	ENGINE_API virtual bool ShouldCache(const FShaderType* ShaderType, const FVertexFactoryType* VertexFactoryType) const;
	...
	{
		FMaterialShaderMapId* OutId = nullptr; // Guaranteed non-null in BuildShaderMapIdOverride
	...
	UE_DEPRECATED(5.7, "Call BuildShaderMapId instead")
	virtual void GetShaderMapId(EShaderPlatform Platform, const ITargetPlatform* TargetPlatform,
		FMaterialShaderMapId& OutId) const
	{
		BuildShaderMapId(OutId, TargetPlatform);
	}

	/** Returns the ShaderMapId used in DDC and IncrementalCook for the ShaderMap created for this FMaterial. */
	UE_DEPRECATED(5.7, "Call BuildShaderMapId without EShaderPlatform as its now cached inside the FMaterial")
	void BuildShaderMapId(FMaterialShaderMapId& OutId, EShaderPlatform Platform, const ITargetPlatform* TargetPlatform,
		EMaterialKeyInclude IncludeFlags = EMaterialKeyInclude::All) const
	{
		BuildShaderMapId(OutId, TargetPlatform, IncludeFlags);
	}
	
	void BuildShaderMapId(FMaterialShaderMapId& OutId, const ITargetPlatform* TargetPlatform,
		EMaterialKeyInclude IncludeFlags = EMaterialKeyInclude::All) const
	{
		BuildShaderMapIdOverride({
			.OutId = &OutId, .Platform = GetShaderPlatform(), .TargetPlatform = TargetPlatform, .IncludeFlags = IncludeFlags }
			);
	}

protected:
	/**
	 * The function that should be override by subclasses that want to modify either the DDC key or the
	 * IncrementalCook key. */
	ENGINE_API virtual void BuildShaderMapIdOverride(const FBuildShaderMapIdArgs& Args) const;

public:

#if WITH_EDITORONLY_DATA
	UE_DEPRECATED(5.7, "Call GetStaticParameterSet without EShaderPlatform as its now cached inside the FMaterial")
	ENGINE_API	virtual void GetStaticParameterSet(EShaderPlatform Platform, FStaticParameterSet& OutSet) const final;
	ENGINE_API virtual void GetStaticParameterSet(FStaticParameterSet& OutSet) const;
	...
	virtual	bool UsesTemporalResponsiveness() const { return false; }
	virtual	bool UsesMotionVectorWorldOffset() const { return false; }
	...
	virtual bool IsUsedWithVoxels() const { return false; }
	virtual bool IsUsedWithVolumetricCloud() const { return false; }
	virtual bool IsUsedWithHeterogeneousVolumes() const { return false; }
	virtual bool SamplesMaterialCache() const { return false; }
	virtual bool HasMaterialCacheOutput() const { return false; }
	...
	UE_DEPRECATED(5.7, "This is not longer valid since even when not plugged in, the default value on the root node can be used")
	...
	UE_DEPRECATED(5.7, "Call IsDualBlendingEnabled without EShaderPlatform as its now cached inside the FMaterial")
	virtual bool IsDualBlendingEnabled(EShaderPlatform Platform) const final { return false; }
	virtual bool IsDualBlendingEnabled() const { return false; }
	...
	UE_DEPRECATED(5.7, "Call GetUniqueAssetName without EShaderPlatform as its now cached inside the FMaterial")
	ENGINE_API FString GetUniqueAssetName(EShaderPlatform Platform, const FMaterialShaderMapId& ShaderMapId) const;
	ENGINE_API FString GetUniqueAssetName(const FMaterialShaderMapId& ShaderMapId) const;
#else
	...
	FString GetUniqueAssetName(const FMaterialShaderMapId& ShaderMapId) const { return GetAssetName(); }
#endif
	...
	virtual int32 GetNumMaterialCacheTags() const { return 0; }
	...
	*/
	...
	inline const EShaderPlatform GetShaderPlatform() const 
	{ 
#if WITH_EDITOR
		checkSlow(ShaderPlatform != EShaderPlatform::SP_NumPlatforms); return ShaderPlatform; 
#else
	...
	UE_DEPRECATED(5.7, "Call ShouldCacheShaders without EShaderPlatform as its now cached inside the FMaterial")
	...
	*/
	...
	UE_DEPRECATED(5.7, "Call SetupExtraCompilationSettings without EShaderPlatform as its now cached inside the FMaterial")
	virtual void SetupExtraCompilationSettings(const EShaderPlatform Platform, FExtraShaderCompilerSettings& Settings) const final { SetupExtraCompilationSettings(Settings); }
	...
	UE_DEPRECATED(5.7, "Call SaveShaderStableKeys without EShaderPlatform as its now cached inside the FMaterial")
	...
	void SaveShaderStableKeys(struct FStableShaderKeyAndValue& SaveKeyVal); // arg is non-const, we modify it as we go
	...
	*
	*/
	UE_DEPRECATED(5.7, "Call GetShaderTypes without EShaderPlatform as its now cached inside the FMaterial")
	...
	ENGINE_API void GetShaderTypes(const FPlatformTypeLayoutParameters& LayoutParams, TArray<FDebugShaderTypeInfo>& OutShaderInfo) const;

	UE_DEPRECATED(5.7, "Call GetShaderTypesForLayout without EShaderPlatform as its now cached inside the FMaterial")
	...
	void GetShaderTypesForLayout(const FShaderMapLayout& Layout, FVertexFactoryType* VertexFactory, TArray<FDebugShaderTypeInfo>& OutShaderInfo) const;
#endif // WITH_EDITOR
...
#else
	inline void SetOwnerBeginDestroyed() {}
	inline bool IsOwnerBeginDestroyed() const { return false; }
#endif
...
protected:
	...
	*/
	...
	UE_DEPRECATED(5.7, "Please use SetQualityLevelProperties with EShaderPlatform argument and not ERHIFeatureLevel::Type")
	...
	{
		SetQualityLevelProperties(GetFeatureLevelShaderPlatform_Checked(InFeatureLevel), InQualityLevel);
	}
...
#if WITH_EDITOR
		FeatureLevel = FDataDrivenShaderPlatformInfo::GetMaxFeatureLevel(InShaderPlatform);
	...
	UE_DEPRECATED(5.7, "Call GetDependentShaderAndVFTypes without EShaderPlatform as its now cached inside the FMaterial")
...
private:
...
#if WITH_EDITOR
	/** Shader Platform that this material is representing. */
	EShaderPlatform ShaderPlatform;
#endif
	...
	*/
	...
	void GetExpressionIncludesHash(FSHAHash& OutHash) const;
...
public:
	...
	// FMaterial interface.
protected:
	ENGINE_API virtual void BuildShaderMapIdOverride(const FBuildShaderMapIdArgs& Args) const override;
public:
#if WITH_EDITORONLY_DATA
	ENGINE_API virtual void GetStaticParameterSet(FStaticParameterSet& OutSet) const override;
#endif
#if WITH_EDITOR
	...
	ENGINE_API virtual bool UsesTemporalResponsiveness() const override;
	ENGINE_API virtual bool UsesMotionVectorWorldOffset() const override;
	...
	ENGINE_API virtual bool IsUsedWithVoxels() const override;
	...
	ENGINE_API virtual bool SamplesMaterialCache() const override;
	ENGINE_API virtual bool HasMaterialCacheOutput() const override;
	...
	ENGINE_API virtual bool IsDualBlendingEnabled() const override;
	...
	ENGINE_API virtual int32 GetNumMaterialCacheTags() const override;
