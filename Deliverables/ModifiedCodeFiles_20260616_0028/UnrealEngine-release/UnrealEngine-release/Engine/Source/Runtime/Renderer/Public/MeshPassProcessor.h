...
inline const TCHAR* GetMeshPassName(EMeshPass::Type MeshPass)
{
	...
	case EMeshPass::ToonOutline: return TEXT("ToonOutline");
#if WITH_EDITOR
...
#if WITH_EDITOR
	static_assert(EMeshPass::Num == 40 + 4, "Need to update switch(MeshPass) after changing EMeshPass"); // GUID to prevent incorrect auto-resolves, please change when changing the expression: {1F4B8E2A-9C77-4D61-8B3E-7A2D5C0E91F3}
#else
	static_assert(EMeshPass::Num == 40, "Need to update switch(MeshPass) after changing EMeshPass"); // GUID to prevent incorrect auto-resolves, please change when changing the expression: {1F4B8E2A-9C77-4D61-8B3E-7A2D5C0E91F3}
#endif
...
struct FMeshDrawCommandStatsData
{
	...
	FName CategoryName;
	/** Total number of vertices for the mesh draw command */
	uint32 NumVertices;
#endif
};
