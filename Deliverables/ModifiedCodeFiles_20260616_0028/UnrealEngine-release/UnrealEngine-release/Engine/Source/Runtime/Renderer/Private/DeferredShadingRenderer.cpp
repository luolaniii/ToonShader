...
void FDeferredShadingSceneRenderer::Render(FRDGBuilder& GraphBuilder, const FSceneRenderUpdateInputs* SceneUpdateInputs)
{
	...
	else
	{
		...
		if (bRenderDeferredLighting)
		{
			...
			// Toon outline: opaque lighting is now composited into scene color; draw inverted-hull
			// outlines before volumetric fog so fog and translucency sort correctly against them.
			RenderToonOutlinePass(GraphBuilder, Views, SceneTextures);
