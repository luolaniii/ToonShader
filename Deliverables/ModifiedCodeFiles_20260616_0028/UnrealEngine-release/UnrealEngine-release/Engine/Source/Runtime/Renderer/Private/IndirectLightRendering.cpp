...
static TAutoConsoleVariable<float> CVarToonIndirectScale(
	TEXT("r.Toon.IndirectScale"),
	0.0f,
	TEXT("Scales indirect lighting (Lumen GI, Lumen reflections, sky light diffuse, SSGI) received by Toon shading model pixels.\n")
	TEXT(" 0: fully isolate toon materials from indirect lighting (default; advanced requirement 2)\n")
	TEXT(" 1: toon materials receive indirect lighting like default-lit (useful for A/B verifying the mask)"),
	ECVF_RenderThreadSafe);
...
class FDiffuseIndirectCompositePS : public FGlobalShader
{
	...
	using FPermutationDomain = TShaderPermutationDomain<FApplyDiffuseIndirectDim, FUpscaleDiffuseIndirectDim, FScreenBentNormalMode, FShortRangeGI, Substrate::FSubstrateTileType, FEnableDualSrcBlending, FScreenSpaceReflectionTiledComposition, FHairComplexTransmittance>;
	...
	static bool ShouldCompilePermutation(const FGlobalShaderPermutationParameters& Parameters)
	{
		...
		if (PermutationVector.Get<FApplyDiffuseIndirectDim>() != 2 
			&& (PermutationVector.Get<FScreenBentNormalMode>() != 0 || PermutationVector.Get<FShortRangeGI>()))
		{
			return false;
		}

		// Build Substrate tile permutation only for Lumen
		if (PermutationVector.Get<Substrate::FSubstrateTileType>() != ESubstrateTileType::EComplex)
		{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(float, ToonIndirectScale)
...
class FReflectionEnvironmentSkyLightingPS : public FGlobalShader
{
	...
	static FPermutationDomain RemapPermutation(FPermutationDomain PermutationVector)
	{
		...
		if (PermutationVector.Get<Substrate::FSubstrateTileType>() && !Substrate::IsSubstrateEnabled())
		{
	...
	FAsyncLumenIndirectLightingOutputs& AsyncLumenIndirectLightingOutputs)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		...
		auto ApplyDiffuseIndirect = [&](ESubstrateTileType TileType)
		{
			...
			LumenReflections::SetupCompositeParameters(View, ViewPipelineState.ReflectionsMethod, PSParameters->ReflectionsCompositeParameters);
			...
			PSParameters->ToonIndirectScale = CVarToonIndirectScale.GetValueOnRenderThread();
			...
			PermutationVector.Set<Substrate::FSubstrateTileType>(ESubstrateTileType::EComplex);
			...
			if (DenoiserOutputs.Textures[0])
			{
				if (ViewPipelineState.DiffuseIndirectMethod == EDiffuseIndirectMethod::Lumen)
				{
					...
					PermutationVector.Set<FDiffuseIndirectCompositePS::FShortRangeGI>(ScreenBentNormalParameters.ShortRangeGITexture != nullptr);
		...
		if (bApplyDiffuseIndirect)
		{
			if (Substrate::IsSubstrateEnabled())
			{
				if (Substrate::GetSubstrateUsesTileType(View, ESubstrateTileType::EComplexSpecial)) 	{ ApplyDiffuseIndirect(ESubstrateTileType::EComplexSpecial);}
				if (Substrate::GetSubstrateUsesTileType(View, ESubstrateTileType::EComplex)) 			{ ApplyDiffuseIndirect(ESubstrateTileType::EComplex);       }
				if (Substrate::GetSubstrateUsesTileType(View, ESubstrateTileType::ESingle)) 			{ ApplyDiffuseIndirect(ESubstrateTileType::ESingle);        }
				if (Substrate::GetSubstrateUsesTileType(View, ESubstrateTileType::ESimple)) 			{ ApplyDiffuseIndirect(ESubstrateTileType::ESimple);        }
			}
			else
			{
		...
		if (IsAmbientCubemapPassRequired(View) && !bIsVisualizePass)
		{
			...
			if (Substrate::IsSubstrateEnabled())
			{
				if (Substrate::GetSubstrateUsesTileType(View, ESubstrateTileType::EComplexSpecial)) 	{ ApplyAmbientCubemapComposite(ESubstrateTileType::EComplexSpecial); }
				if (Substrate::GetSubstrateUsesTileType(View, ESubstrateTileType::EComplex))			{ ApplyAmbientCubemapComposite(ESubstrateTileType::EComplex); }
				if (Substrate::GetSubstrateUsesTileType(View, ESubstrateTileType::ESingle)) 			{ ApplyAmbientCubemapComposite(ESubstrateTileType::ESingle); }
				if (Substrate::GetSubstrateUsesTileType(View, ESubstrateTileType::ESimple)) 			{ ApplyAmbientCubemapComposite(ESubstrateTileType::ESimple); }
			}
			else
			{
	...
	ESubstrateTileType SubstrateTileMaterialType)
{
	...
	// Setup the parameters of the shader.
	{
		...
		LumenReflections::SetupCompositeParameters(View, ReflectionsMethod, PassParameters->PS.ReflectionsCompositeParameters);
	}
	...
	const bool bSubstrateEnabled = Substrate::IsSubstrateEnabled();
	const bool bSubstrateLightingPassesTiled = Substrate::AreLightingPassesTiled();
	if (bSubstrateEnabled && bSubstrateLightingPassesTiled)
	{
	...
	GraphBuilder.AddPass(
		...
		SubstrateTilePassVertexShader, bSubstrateEnabled, SubstrateTilePrimitiveType, bSubstrateLightingPassesTiled](FRDGAsyncTask, FRHICommandList& InRHICmdList)
	{
		...
		if (bSubstrateEnabled && !bSubstrateLightingPassesTiled)
		{
			GetSubstrateTiledDepthStencilStateNoDepthTest(SubstrateTileMaterialType, GraphicsPSOInit, StencilRef);
		}
		...
		SetGraphicsPipelineState(InRHICmdList, GraphicsPSOInit, StencilRef);
